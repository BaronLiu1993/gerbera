...
def test_dcmotor_builder_defines_write_only_driver_contract() -> None:
    ...
    assert f"MCP,{connection.event_name},status:forward" in handler
    assert f"MCP,{connection.event_name},status:reverse" in handler
    assert f"MCP,{connection.event_name},status:stop" in handler
    assert f"MCP,{connection.event_name},error:invalid_direction" in handler
